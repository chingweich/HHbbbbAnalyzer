...
void plotAllMassVariables(std::string inputFile){
 ...
 hmass[i][k]->Fit(tf1[0],"","",th1->GetMaximumBin()-20,th1->GetMaximumBin()+20);
...
tf1[1] ->SetLineColor(COLORS[i]);
 ...
 hmassFit[3][i][k]=tf1[0]->GetParError(2);
		...
		tf1[1]=new TF1("fa1","gaus(25000)",th1->GetMaximumBin()-0.2,th1->GetMaximumBin()+0.2);
...
hdiffmass[i][k]->Fit(tf1[1],"","",th1->GetMaximumBin()-0.2,th1->GetMaximumBin()+0.2);
