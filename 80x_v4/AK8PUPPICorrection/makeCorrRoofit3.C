...
void makeCorrRoofit3(){
	...
	for(int i=0;i<nMasspoint;i++){
		...
		RooRealVar meanBi("mean","mean",109,90,120) ;
		RooRealVar sigmaR("sigmaR","sigmaR",9,5,15) ;
		RooRealVar sigmaL("sigmaL","sigmaL",16,10,30) ;
		RooBifurGauss Bi("bi","bi",x,meanBi,sigmaL,sigmaR);
		
		
		RooRealVar m0("m0","m0",107.9,90,125);
		RooRealVar sig("sigma","sigma",10,5,30);
		RooRealVar alpha("alpha","alpha",0.9,0,10);
		RooRealVar n("n","n",6.6,0,20);
