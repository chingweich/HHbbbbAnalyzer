...
void makeCorrRoofit2(){
	...
	int masspoint[nMasspoint]={600,800,1000,1200,1400,1600,1800,2000,2500,3000};
	...
	for(int i=0;i<nMasspoint;i++){
		...
		RooRealVar meanB("mean","mean",80,50,110) ;
		RooRealVar sigmaR("sigmaR","sigmaR",5,0.1,9) ;
		RooRealVar sigmaL("sigmaL","sigmaL",5,0.1,18) ;
		RooBifurGauss Bi("bi","bi",x,meanB,sigmaL,sigmaR);
		...
		dh.plotOn(frame);
		Bi.fitTo(dh) ; 
		Bi.plotOn(frame,LineColor(kRed)) ;
		Bi.paramOn(frame,Layout(0.55)) ;
		mean[0][i]=80/meanB.getValV();
		sigma[0][i]=meanB.getError()*(80/meanB.getValV())/meanB.getValV();
	...
	for(int i=0;i<nMasspoint;i++){
		...
		Bi.plotOn(frame,LineColor(kRed)) ;
		Bi.paramOn(frame,Layout(0.55)) ;
		mean[1][i]=80/meanB.getValV();
		sigma[1][i]=meanB.getError()*(80/meanB.getValV())/meanB.getValV();
		//mean[4][i]=125/th1->GetMean();
		//sigma[4][i]=th1->GetMeanError()/th1->GetMean();
		//th1->Draw();
		frame->SetMaximum(th1->GetMaximum()*1.3);
