...
void plotAllMassVariablesBase(std::string inputFile,int print=0){
  ...
  std::string prefix[]={""};
  std::string name[]={"uncorr_l","uncorr_s","corr_l","corr_s"};
  ...
  for(int i=0; i < NTYPES;i++){
	...
	//hmass[i][k]->Sumw2();
      ...
      hmass[i][k] = (TH1F*)inf->FindObjectAny(Form("%s",name[i].data(),prefix[k].data()));
	hmass[i][k]->Sumw2();
...
//tf1[0] ->SetLineColor(COLORS[i]);
 ...
 hmassFit[3][i][k]=tf1[0]->GetParError(2);
	...
	//hdiffmass[i][k]->Sumw2();
      ...
      hdiffmass[i][k] = (TH1F*)inf->FindObjectAny(Form("%s",name[i].data(),prefix[k].data()));			
hdiffmass[i][k]->Sumw2();	
...
hdiffmassFit[0][i][k]=tf1[1]->GetParameter(1);
 ...
 hdiffmassFit[3][i][k]=tf1[1]->GetParError(2);
  ...
  TLegend* leg = new TLegend(0.179,0.192,0.326,0.882);
  leg->SetFillColor(0);
  leg->SetFillStyle(0);
  ...
  for(int k=0; k < NHISTOS; k++){
	...
	leg->AddEntry((TObject*)0, Form("Mean=%f",hmassFit[0][i][k]),"");
      leg->AddEntry((TObject*)0, Form("#sigma=%f",hmassFit[2][i][k]),"");
      if(i==NTYPES-1)
	leg->Draw("same");
      if(i==NTYPES-1 && nPage==0)
	{
	  if(print==1)c1->Print(Form("%s(",outputFile.Data()),"pdf");
	  else if (print ==2)c1->Print(Form("%s)",outputFile.Data()),"pdf");
	 else  c1->Print(Form("%s",outputFile.Data()),"pdf");
	  nPage++;
	}
      else if(i==NTYPES-1)
	{
...
plotAllMassVariablesBase("corr2/B1000.root",1);
plotAllMassVariablesBase("corr2/B1200.root");
plotAllMassVariablesBase("corr2/B1400.root");
plotAllMassVariablesBase("corr2/B1600.root");
plotAllMassVariablesBase("corr2/B1800.root");
plotAllMassVariablesBase("corr2/B2000.root");
plotAllMassVariablesBase("corr2/B2500.root");
plotAllMassVariablesBase("corr2/B3000.root",2);
