...
void makeCategory(){
	...
	for (int i=0;i<3;i++){
		for(int j=0;j<26;j++){
			...
			if(i==0 )th1[i][j]=(TH1D*)f[j]->FindObjectAny(Form("AK8PuppijetP4RegHCorr_%d_%d",bmin[m],bmin[m]+width[k]));
			if(i==1 )th1[i][j]=(TH1D*)f[j]->FindObjectAny(Form("AK8PuppijetP4RegHCorrLL_%d_%d",bmin[m],bmin[m]+width[k]));
