...
void variableComparison(){
	...
	st.push_back("unjet1pt"); up.push_back(800);  down.push_back(100);  
	st.push_back("unjet2pt"); up.push_back(800);  down.push_back(100);  
	st.push_back("unjet1eta"); up.push_back(3);  down.push_back(-3);  
	st.push_back("unjet2eta"); up.push_back(3);  down.push_back(-3);  
	...
	TFile* tf1[2];
	tf1[0]=TFile::Open("prompt.root");
	tf1[1]=TFile::Open("re.root");
	...
	for(unsigned int j=0;j<st.size()-1;j++){
		...
		th1[0]=(TH1D* )tf1[0]->Get(Form("%s",st[j].data()));
		th1[1]=(TH1D* )tf1[1]->Get(Form("%s",st[j].data()));
