...
void miniTreeBase(int wMs,int wM, string st,string st2,string option="") {
	...
	for (int w=wMs;w<wM;w++){
		...
		//cout<<"here"<<endl;
		TFile* outFile = new TFile(Form("minitree/%s_%d.root",st2.data(),w),"recreate");
...
void miniTree_v3_1(int a,int b){
	//miniTreeBase(1,2,"BulkGravTohhTohbbhbb_narrow_M-2000_13TeV-madgraph.root","miniTreeTest");
	//miniTreeBase(1,2,input,output);
	
	
	string st1[40]={
		"/afs/cern.ch/work/c/chchen/DM/skimm/CMSSW_8_0_22/src/scripts/macro_examples/dihiggs/lxplus_batch/skimming/dataB/skimmed_NCUGlobalTuples_",
	...
	miniTreeBase(b,b+1,st1[a],fileName[a]);	
}
