...
TH1D* massPlotBase(string input,string variable,double down=100,double up=1000,int category=1){
	...
	for(Long64_t jEntry=0; jEntry<data.GetEntriesFast() ;jEntry++){
			...
			if((jet1bbtag<0.3 ||jet2bbtag<0.3 )&& category==2)continue;
			
			if((jet1bbtag>0.3 &&jet2bbtag>0.8 )&& category==2)continue;
...
TH1D* massPlotBaseSF(string input,string variable,double down=100,double up=1000,int category=1){
	...
	for(Long64_t jEntry=0; jEntry<data.GetEntriesFast() ;jEntry++){
			...
			count++;
			//Float_t  jet2_puppi_msoftdrop = data.GetFloat("jet2_puppi_msoftdrop");
			if(dbtSF>3||dbtSF<0)continue;
			if(dbtSF<0.1||dbtSF>2)cout<<"dbtSF="<<dbtSF<<endl;
...
void reducedMassPlots(){
	...
	for(int i=0;i<13;i++){
		th1[0]=massPlotBase(Form("BulkGrav_M-%d_0.root",massP[i]),"dijetmass_softdrop_corr",massP[i]*0.7,massP[i]*1.3);
		th1[1]=massPlotBaseSF(Form("BulkGrav_M-%d_0.root",massP[i]),"dijetmass_softdrop_corr",massP[i]*0.7,massP[i]*1.3);
