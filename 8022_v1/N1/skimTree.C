...
void skimTree(int w , string st){
	...
	for(int i=8;i<11;i++)th1[i]=new TH1D(Form("%s",str[i-4].data()),Form("%s",str[i-4].data()),nbins[i-4],down[i-4],up[i-4]);
	...
	for(int i=0;i<12;i++)fixScale->SetBinContent(i+1,fixScaleNum[i]);
