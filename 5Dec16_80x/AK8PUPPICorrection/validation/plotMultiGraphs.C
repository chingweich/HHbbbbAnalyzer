...
void plotMultiGraphs(){
  ...
  const int NTYPES=5;
  float mass[masspoint]={487.441,568.772,665.847,761.498,842.68,919.298,1152.37,1345.38};
 // int MARKERS[7]={20,21,22,23,34,29,24};
  ...
  for(int i=0; i<3; i++){
	...
	graph_mean[j] = new TGraphErrors(masspoint,mass,mean,masserr,meanerr);
	...
	graph_RMS[j] = new TGraphErrors(masspoint,mass,RMS,masserr,RMSerr);
	...
	graph_RMSMean[j] = new TGraphErrors(masspoint,mass,rel,masserr,relerr);
