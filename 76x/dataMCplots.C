...
*/
  ...
  lar->DrawLatex(0.60, 0.94, "L = 2.6937 fb^{-1} at #sqrt{s} = 13 TeV");
...
h_name.push_back("Nbtagjet");  
  ...
  for(unsigned int i = 0; i < h_name.size(); i++){
	...
	vector<TH1D* > v2;
	vector<TH1D* > vd;
	double fixNumber=7124/10366.3;//7306/11857.3;
	...
	for(int k=0;k<4;k++){
		...
		scaleTemp[k]=fixNumber*2693.7*Xsec[k]/th2->GetBinContent(1);
		v2.push_back(th1[k]);
