...
*/
  ...
  int bmin=0,bmax=0;
	...
	h_data->GetXaxis()->SetRangeUser(rangeUserDown,rangeUserUp);
...
void myRatio(TH1D* h_data, TH1D *h_bkg){
  ...
  one->SetLineWidth(2);
  one->Draw("same");
	h_ratio->GetXaxis()->SetRangeUser(rangeUserDown,rangeUserUp);
  h_ratio->Draw("same");
...
void dataMCplots(){
  ...
  for(unsigned int i = 0; i < h_name.size(); i++){
	...
	vector<TH1D* > v2;
	vector<TH1D* > vd;
	double fixNumber=7306/11857.3;
	double Xsec[4]={6831,1207,119.9,25.24};
	for(int k=0;k<4;k++){
		TH1D *th2=(TH1D* )tf1[k]->FindObjectAny("cutflow");
		
		th1[k]->Scale(fixNumber*2693.7*Xsec[k]/th2->GetBinContent(1));
		v2.push_back(th1[k]);
...
vd,
   ...
   // c.Draw();
	...
	if(h_name[i].find("1b")!= std::string::npos)c.Print(Form("dataMC/1b/%s.pdf",h_name[i].data()));
	if(h_name[i].find("2b")!= std::string::npos)c.Print(Form("dataMC/2b/%s.pdf",h_name[i].data()));
    if(h_name[i].find("4b")!= std::string::npos)c.Print(Form("dataMC/all/%s.pdf",endfix.Data()));
  }
