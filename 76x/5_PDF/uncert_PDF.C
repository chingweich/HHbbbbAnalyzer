...
void uncert_PDF(){
	...
	for (int i=0;i<11;i++)tf3[i]=TFile::Open(Form("../sf/B%s_tau21Up.root",st[i].data()));
	for (int i=0;i<11;i++)tf5[i]=TFile::Open(Form("../sf/B%s_tau21Down.root",st[i].data()));
	...
	for (int i=0;i<11;i++){
		TH1D* th7[14];
		th7[0]=new TH1D("totalMass","totalMass",200,0,1);
		th7[1]= (TH1D *)tf1[i]->FindObjectAny("PDFEff");
		for(int j=2;j<102;j++)th7[0]->Fill(th7[1]->GetBinContent(j));
		
		uns[i]=th7[0]->GetRMS()/th7[1]->GetBinContent(1);
		cout<<i<<","<<uns[i]<<endl;
	...
	TGraph * tg4=new TGraph(10,xx,unss);
	
	tg4->SetTitle("PDF uncertainty");
	tg2->SetTitle("tau21_up fit");
	tg3->SetTitle("tau21_down fit");
	tg1->SetLineColor(1);
	tg2->SetLineColor(2);
	tg3->SetLineColor(3);
	...
	//tg1->SetMaximum(1.002);
//	tg1->SetMinimum(0.998);
	//tg1->Draw("APL");
	//tg2->Draw("PL,same");
//	tg3->Draw("PL,same");
	...
	leg->AddEntry(tg1,"tau21_central");
	leg->AddEntry(tg2,"tau21_up");
	leg->AddEntry(tg3,"tau21_down");
	...
	tg4->Draw("APL");
	tg1->GetXaxis()->SetTitle("M_{X}[GeV]");
	tg1->GetYaxis()->SetTitle("uncertainty");	
