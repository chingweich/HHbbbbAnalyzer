...
void draw(){
	...
	for(int i=0;i<4;i++){
		...
		TH1D* fixScale=(TH1D*)f[i+22]->FindObjectAny("fixScale");
		norm[i]=5884.429253654*Xsec[i]/fixScale->GetBinContent(1);
		ct[i]->Scale(norm[i]);
	}
	...
	for(int i=0;i<4;i++){
		ct[i]=(TH1D*)f[i+22]->FindObjectAny("NbtagjetB");
		ct[i]->Scale(norm[i]);
	}
	...
	double total =0;
	for (int i=0;i<4;i++)for (int j=0;j<5;j++)total+=ct[i]->GetBinContent(j+1);
	cout<<total;
