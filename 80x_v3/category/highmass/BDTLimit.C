...
void BDTLimit(){
	...
	f[2]=TFile::Open("Graviton_subtrMMe.root");
	f[3]=TFile::Open("Graviton_subtrMLe.root");
...
string st[7]={"TM","TL","MM","ML","LL","TT","3b+4b"};
	...
	for(int i=0;i<7;i++){
		...
		if(i<5)leg->AddEntry(tg1[i],Form("%s+TT",st[i].data()));
		else leg->AddEntry(tg1[i],Form("%s",st[i].data()));
	}
	leg->Draw("same");
	
	//c1->SetLogy();
	c1->Print("BDTLimitcombine.pdf");
}
