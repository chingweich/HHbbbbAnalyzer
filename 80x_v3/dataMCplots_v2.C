  ...
  TString endfix;
	...
	if(h_name[i].find("4b")!= std::string::npos){
		...
		for(int j=0;j<5;j++){
				...
				TH1D *th2;
				th2=(TH1D* )tf1[j]->FindObjectAny(Form("%s_0b",endfix.Data()));
				th1[j]->Add(th2);
...
h_name[i].find("Eta")!= std::string::npos ||
h_name[i].find("double")!= std::string::npos ){
	  for(int k=0;k<5;k++)th1[k]->Rebin(2);
  }
