...
void draw(){
  ...
  leg->SetTextSize(0.04);
		...
		for(int j=0;j<3;j++){
			//cout<<i<<endl;
				th1[j]=(TH1D*)f->Get(Form("%s_both",massName[j].data()));
				//th2[j]=(TH1D*)f->Get(Form("%s_leading",massName[j].data()));
				
				//cout<<i<<","<<th2[j]->Integral()<<","<<th1[j]->Integral();
				//th1[i]->Add(th2[j]);
		...
		if(i==0)c1->Print("both.pdf(");
		else if(i==9)c1->Print("both.pdf)");
		else c1->Print("both.pdf");
	}
