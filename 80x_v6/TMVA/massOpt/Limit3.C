...
void Limit3(){
  ...
  string categoryName[7]={"0a","0c","1a","1c","2a","2b","2d"};
	...
	for(int h=2;h<7;h++){
		...
		TFile* tf2=TFile::Open(Form("%s/MassPlotFineBins_subtr_Moriond_Silver105to135.root",categoryName[h].data()));
			 if (!tf1 || !tf1->IsOpen())continue;
			 TGraphAsymmErrors* tg2=(TGraphAsymmErrors*)tf1->Get("LimitExpectedCLs");
			 TGraphAsymmErrors* tg1=(TGraphAsymmErrors*)tf2->Get("LimitExpectedCLs");
