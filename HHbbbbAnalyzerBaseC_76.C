void HHbbbbAnalyzerBaseC_76(int wMs,int wM, string st,string st2,double Xsec,int nameRoot=0){	
	TFile *f;
	TTree *tree;
	...
	TFile* outFile = new TFile(Form("root_files_7476/%s_2.root",st2.data()),"recreate");
	if(nameRoot==0){
		for(int i=0;i<40;i++){
			//th1[i]->Sumw2();
